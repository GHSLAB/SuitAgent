...
> **目录结构定义**：详见 [`.claude/config/case-directories.yaml`](.claude/config/case-directories.yaml)
>
...
│   ├── 03 - 🔍 法律研究/             # IssueIdentifier + Researcher输出
│   ├── 04 - 📤 客户提供/             # 客户主动提供的材料
│   ├── 05 - 📎 证据材料/             # EvidenceAnalyzer输出
│   ├── 06 - 📝 法律文书/             # Writer输出（所有文书直接存放，不创建子目录）
│   ├── 07 - 📥 对方提交/             # 对方当事人提交的材料
│   ├── 08 - 🏛️ 法院送达/             # 法院送达文书
│   ├── 09 - 🎯 庭审笔录/             # 庭审过程记录
...
**核心原则**：所有时间相关的信息必须基于系统本机时间，严禁使用虚构或估算的时间戳。

#### 1. 时间获取强制要求

**当处理任何涉及时间的文档或案件信息时，必须：**
...
| 材料类型 | 识别特征 | 放置位置 |
|----------|----------|----------|
| **起诉状** | "原告"、"被告"、"诉讼请求" | `02 - 📄 案件分析/` |
| **证据材料** | "证据"、"附件"、"证明目的" | `05 - 📎 证据材料/` |
| **庭审笔录** | "庭审"、"审判员"、"当事人" | `02 - 📄 案件分析/` |
| **判决书** | "判决"、"裁定"、"法院认为" | `02 - 📄 案件分析/` |
| **法律文书** | "答辩状"、"代理词"等 | `06 - 📝 法律文书/` |
| **工作记录** | "工时"、"工作内容"、"期限" | `00 - 📅 日程管理/` |
...
#### 1. 信息来源验证原则
- **绝对禁止**: 编造、猜测或推断任何当事人信息
- **必须要求**: 所有关键信息必须有明确的文档来源
- **拒绝填写**: 无法确认的信息必须标注"[待确认]"或"[待获取]"

#### 2. 多文档交叉验证机制
当处理案件当事人信息时，必须：
```bash
# 1. 检查文档一致性
if multiple_docs_available:
...
**核心原则**: 无法读取到的信息直接留空，禁止猜测填补

**常见留空场景**:
- **法院案号**: 如未立案，标注`[待法院立案]`或直接留空
- **律所案号**: 如委托材料中未明确，标注`[待确认]`或留空
- **立案日期**: 无法从文档中获取确切开庭日期时，留空处理
- **律师执业证号**: 委托材料中未记录时，标注`[待获取]`
- **当事人地址/电话**: 委托材料中缺失时，标注`[待确认]`
- **法定代表人信息**: 营业执照未提供时，标注`[待获取]`

**填写规范**:

```markdown
错误做法: 律师执业证号: 1234567890123456 (编造)
正确做法: 律师执业证号: [待获取]

错误做法: 立案日期: 2025-11-12 (猜测)
正确做法: 立案日期: [待立案后确认]

错误做法: 法院案号: [2025]苏0591民初1234号 (编造)
正确做法: 法院案号: [待法院立案] 或留空
```
...
- **OCR识别失败**: 明确报告"OCR无法识别，建议人工核对"
- **文档损坏**: 指出具体文档和损坏情况
- **信息矛盾**: 列出矛盾文档，建议人工确认
- **信息缺失**: 严格遵循"留空待填写"原则，不能填补

#### 5. 信息提取优先级
按照以下优先级提取信息：
1. **委托代理合同** - 身份证号、律师信息、费用
2. **授权委托书** - 代理权限、律师姓名
3. **身份证PDF** - OCR识别身份证信息
4. **租赁协议** - 双方当事人信息、租赁条款
5. **其他证据材料** - 补充验证信息
...
> **Agent与目录的完整映射关系**：详见 [`.claude/config/agent-mappings.yaml`](.claude/config/agent-mappings.yaml)
>
> 该配置文件定义了所有10个Agent的输出目录映射，按四层架构组织。
>
> 目录名称来自 `case-directories.yaml` 的 `folder_name` 字段。

每个Agent都应将输出保存到对应的专门文件夹中。具体的目录映射关系请参考配置文件。

### 🚨 主Agent MCP强制检查规范

#### 核心禁止原则
**主Agent绝对禁止直接调用PDF/图片处理技能**，必须通过专门的Agent处理：

- ❌ **禁止**: 主Agent直接使用 `pdf` skill
- ❌ **禁止**: 主Agent直接调用 `mcp_mineru` 工具
- ❌ **禁止**: 绕过DocAnalyzer直接处理PDF/图片文档
- ✅ **必须**: 调用DocAnalyzer Agent处理PDF/图片
- ✅ **必须**: 让DocAgent在内部进行MCP检查和调用

#### 强制检查流程
当主Agent需要处理PDF/图片文档时，必须执行以下步骤：

1. **需求识别** → 识别需要处理PDF/图片文档
2. **Agent调用** → 调用DocAnalyzer Agent（不直接处理）
3. **内部处理** → DocAnalyzer内部进行MCP配置检查
4. **优先级处理** → DocAnalyzer优先使用MCP，失败时回退到技能
5. **结果接收** → 接收DocAnalyzer的结构化输出

#### 违规检测与纠正机制
**自动检测**：
- 主Agent每次使用PDF技能时，必须记录违规行为
- 系统应建立检测机制识别直接调用技能的行为

**强制纠正**：
- 发现违规时，立即停止当前操作
- 重新通过DocAnalyzer Agent处理文档
- 在更新历史中记录违规行为和纠正过程

#### 质量保障清单
主Agent在处理文档前必须确认：
- [ ] 文档类型是否为PDF/图片
- [ ] 是否已调用DocAnalyzer Agent
- [ ] 是否未直接使用PDF技能
- [ ] 是否未直接调用MCP工具
- [ ] 已将处理任务完全委托给DocAnalyzer

### 🎯 MCP工具调用原则

### PDF/图片文档处理流程

**核心原则**：所有PDF/图片OCR处理**必须通过DocAnalyzer Agent**，主Agent不应直接调用MCP工具。

**正确流程**：
```
用户上传PDF → 主Agent识别需求 → 调用DocAnalyzer Agent → DocAnalyzer检查MCP配置 →
优先使用MCP (mineru.parse_documents()) → 如失败则回退到PDF skill → 生成结构化输出
```

**DocAnalyzer的混合模式**：
1. **检查MCP配置**：查看 `.claude/mcp.json` 是否存在mineru配置
2. **配置存在？** → 使用MCP工具（mineru.parse_documents()）
3. **配置不存在或调用失败？** → 回退到PDF skill
4. **统一输出**：无论使用MCP还是PDF skill，生成相同格式的Markdown输出

### 📋 质量保证

当DocAnalyzer处理PDF时检查：

- [ ] 是否检查了MCP配置
- [ ] 是否优先尝试了MCP（如配置存在）
- [ ] MCP失败时是否自动回退到PDF skill
- [ ] 是否生成了标准化的Markdown文件
- [ ] 输出中是否记录了使用的OCR工具
