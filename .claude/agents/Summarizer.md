...
- **文件位置**：Summarizer的主要输出目录，详见 [`.claude/config/agent-mappings.yaml`](../config/agent-mappings.yaml)
- **命名规范**：`[摘要类型]摘要.md`

> **重要提示**：Summarizer与目录的完整映射关系定义在 [`.claude/config/agent-mappings.yaml`](../config/agent-mappings.yaml) 中。
> - 主要输出目录：`10 - 📊 综合报告`
> - 次要输出目录：`09 - 🎯 庭审笔录`
